...
(define %xous-commit "55d44bd947ad1b4bbe58e965dd4834c7706b19d3")

;; SHA256 hash of the git checkout (get with: guix hash -rx <checkout>)
(define %xous-hash "1vnmqizjn5w719lrxzaq2h6hm9gifp0ci9vhgxrsr02axp9x7dng")

;; Version string (e.g., "v0.9.16-0-gabcd1234" or just "v0.9.16")
;; v0.9.16-2719-g55d44bd94
...
(define-public firmware-bao1x-boot0
  (make-firmware-build "firmware-bao1x-boot0" "bao1x-boot0"
                       #:target-dir "riscv32imac-unknown-none-elf"
                       #:crate-inputs (lookup-cargo-inputs 'bao1x-boot0)))

(define-public firmware-bao1x-boot1
  (make-firmware-build "firmware-bao1x-boot1" "bao1x-boot1"
                       #:target-dir "riscv32imac-unknown-none-elf"
                       #:crate-inputs (lookup-cargo-inputs 'bao1x-boot1)))

(define-public firmware-bao1x-alt-boot1
  (make-firmware-build "firmware-bao1x-alt-boot1" "bao1x-alt-boot1"
                       #:target-dir "riscv32imac-unknown-none-elf"
                       #:crate-inputs (lookup-cargo-inputs 'bao1x-alt-boot1)))

(define-public firmware-bao1x-baremetal-dabao
  (make-firmware-build "firmware-bao1x-baremetal-dabao" "bao1x-baremetal-dabao"
                       #:target-dir "riscv32imac-unknown-none-elf"
                       #:crate-inputs (lookup-cargo-inputs 'bao1x-baremetal-dabao)))

(define-public firmware-dabao
  (make-firmware-build "firmware-dabao" "dabao"
                       #:target-dir "riscv32imac-unknown-xous-elf"
                       #:crate-inputs (lookup-cargo-inputs 'dabao)))

(define-public firmware-dabao-helloworld
  (make-firmware-build "firmware-dabao-helloworld" "dabao helloworld"
                       #:target-dir "riscv32imac-unknown-xous-elf"
                       #:crate-inputs (lookup-cargo-inputs 'dabao-helloworld)))

(define-public firmware-baosec
  (make-firmware-build "firmware-baosec" "baosec"
                       #:target-dir "riscv32imac-unknown-xous-elf"
                       #:crate-inputs (lookup-cargo-inputs 'baosec)))

;;; Combined bootloader package
(define-public firmware-bootloader
  (package
    ...
    (inputs
     `(("boot0" ,firmware-bao1x-boot0)
       ("boot1" ,firmware-bao1x-boot1)
       ("alt-boot1" ,firmware-bao1x-alt-boot1)))
    (home-page "https://github.com/betrusted-io/xous-core")
    (synopsis "Combined Xous bootloader package (production)")
    (description (string-append "Production bootloader package containing boot0, boot1, "
