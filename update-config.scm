...
(define option-spec
  '((xous-core-commit (single-char #\x) (value #t))
    (rust-xous-commit (single-char #\r) (value #t))
    (clone-depth      (single-char #\d) (value #t))
    (help             (single-char #\h))))

(define (usage)
  (format #t "Usage: ~a [OPTIONS]~%" (car (command-line)))
  (format #t "~%")
  (format #t "Options:~%")
  (format #t "  -x, --xous-core-commit COMMIT   Update xous-core config~%")
  (format #t "  -r, --rust-xous-commit COMMIT   Update betrusted-io/rust config~%")
  (format #t "  -d, --clone-depth N             Git clone depth (default: ~a)~%" %default-clone-depth)
  (format #t "  -h, --help                      Show this help~%")
  (format #t "~%")
  (format #t "Examples:~%")
  (format #t "  ~a -x abc123~%" (car (command-line)))
  (format #t "  ~a -x abc123 -r def456~%" (car (command-line)))
  (format #t "  ~a --xous-core-commit abc123 --clone-depth 100~%" (car (command-line)))
  (exit 1))
...
(define (update-repo! commit url owner upstream-url clone-depth vars)
  ...
  (let ((tmpdir (make-temp-dir))
    ...
    (dynamic-wind
      ...
      (lambda ()
        (format #t "~%")
        (run-git "clone" "--depth" (number->string clone-depth)
                 "--tags" url tmpdir)
        (chdir tmpdir)
        ...
        (run-git "fetch" "--depth" (number->string clone-depth)
                 "origin" commit)
        ...
        (let ((describe (run-command "git describe --long --abbrev=9"))
          ...
          (when (and (assoc-ref vars 'describe) describe)
            (update-config! (assoc-ref vars 'describe) describe))
...
(define (main args)
  (let* ((options (getopt-long args option-spec))
         (xous-commit (option-ref options 'xous-core-commit #f))
         (rust-commit (option-ref options 'rust-xous-commit #f))
         (clone-depth (or (and=> (option-ref options 'clone-depth #f) string->number)
                          %default-clone-depth))
         (help? (option-ref options 'help #f)))
    (when (or help? (and (not xous-commit) (not rust-commit)))
      (usage))
    ;; Update xous-core
    (when xous-commit
    ...
    (when rust-commit
      ...
      (update-repo!
       ...
       '((commit . "%rust-xous-commit")
         (describe . #f)  ; no git-describe for rust
